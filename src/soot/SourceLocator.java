...
public class SourceLocator
{
    ...
    public Set<String> classesInDynamicPackage(String str) {
        ...
        HashSet<String> set = new LinkedHashSet<String>(0);
    ...
    public static class FoundFile {
        // Begin javarifier changes
        FoundFile( String classpathEntry, ZipFile zipFile, ZipEntry entry ) {
            this.classpathEntry = classpathEntry;
            this.zipFile = zipFile;
            this.entry = entry;
        }
        FoundFile( String classpathEntry, File file ) {
            this.classpathEntry = classpathEntry;
            this.file = file;
        }
        public String classpathEntry; // Javarifier -- check against stubCPEntries
        // End javarifier changes
        public File file;
    ...
    private FoundFile lookupInJar(String jar, String fileName) {
        try {
            ...
            // Begin javarifier changes
            return new FoundFile(jar, jarFile, entry);
            // End javarifier changes
        } catch( IOException e ) {
    ...
    private Map<String, EntryKind> entryKinds = null;
    private void putEntryKind1(String entry, EntryKind kind) {
        try {
            String cnEntry = new File(entry).getCanonicalPath();
            entryKinds.put(cnEntry, kind);
        } catch (IOException e) {
            throw new RuntimeException("Failed to canonicalize " + entry, e);
        }
    }
    private void initEntryKinds() {
        if (entryKinds == null) {
            entryKinds = new HashMap<String, EntryKind>();
            for (Object entry1 : classPath()) {
            ...
            if (programCPEntries1 != null) {
                ...
                for (String entry : programCPEntries)
                    putEntryKind1(entry, EntryKind.PROGRAM);
            }
        }
    }
    public EntryKind entryKindForClass(String className) {
        initEntryKinds();
        String fileName = className.replace('.', File.separatorChar) + ".class";
        FoundFile ff = SourceLocator.v().lookupInClassPath(fileName);
        if (ff == null)
            return null;
        else
            return entryKinds.get(ff.classpathEntry);
    }
    // End javarifier changes
}
