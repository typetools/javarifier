...
class AbstractSootMethodRef implements SootMethodRef {
    ...
    private SootMethod resolve(StringBuffer trace) {
        ...
        while(true) {
            ...
            // If precise method is not found, try ignoring return type and 
            // see if a method with the same signature is included in the class.
            // (Since methods cannot differ only in return type in the same 
            //  class, this procedure is safe as long as this code is inside
            //  the loop, so it finishes checking the current class 
            //  (declaringClass) before going up the hierarchy.
            SootMethod methodIgnoringReturn = 
              cl.getMethodIgnoringReturnType(getSubSignature());
            if(methodIgnoringReturn != null) {
              return methodIgnoringReturn;
            }
