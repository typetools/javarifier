// Change for Javarifier:
// Added getBody and cacheCurrentBody methods.  Currently, soot nulls out the
// body.  This method caches the body for Javarifier to use.
//
// Also added java 1.5 signature fields to the class such as getJrReturnType.
...
public class SootMethod
    ...
    implements ClassMember, Numberable, MethodOrMethodContext, javarifier.Signatured, javarifier.JrTyped {

    // Javarifer Change
    public EntryKind entryKind() {
        return getDeclaringClass().entryKind();
    }

    private Body body;
    public void cacheCurrentBody() {
        this.body = getActiveBody();
    }
    public Body getBody() {
        if (entryKind() != EntryKind.PROGRAM) {
            return null;
        }
        if (isConcrete()) {
            return retrieveActiveBody();
        } else {
            return null;
        }
    }
    ...
    public boolean hasMethodSigSet() {
      return sig != null;
    }
    public MethodSig getSig() {
        if (sig == null) { 
          throw new RuntimeException("SootMethod.getSig: called before sig was set"); }
        return sig;
    }
    public void setSig(MethodSig sig) {
        if (sig == null) {
    ...
    public JrType getJrReturnType() {
        return sig.getReturnType();

    }

    public JrType getJrType() {
        return getJrReturnType();
    }
    ...
    public List<Param> getParameters() {
        if (parameters == null) {
            ...
            MethodSig methodSig = this.getSig();
            List<JrType> methodSigParams = methodSig.getParams();
            boolean innerClassConstructor = false;
            List/*<RefType>*/ paramTypes = this.getParameterTypes();
            innerClassConstructor = paramTypes.size() > methodSigParams.size();
            if(innerClassConstructor) {
              if(!this.name.contains("<init>")) {
                System.out.println("SootMethod.getParameters more paramTypes than params");
              }
            }
            
            if(innerClassConstructor) {
              String outerClassName = ((RefType) paramTypes.get(0)).getClassName();
              outerClassName = outerClassName.replace('.', '/') + ";";
              JrType outerClassParamType = new ClassType(Mutability.UNKNOWN, outerClassName, new ArrayList<TypeArg>());
              params.add(new Param(this, outerClassParamType, 0));
            }
            
            // For inner class constructors, need to add 1 more parameter,
            // the implicit containing class
            int paramOffset = innerClassConstructor ? 1 : 0;
            for (int i = 0; i < getSig().getParams().size(); i++) {
                JrType type = getSig().getParams().get(i);
                params.add(new Param(this, type, i + paramOffset));
            }
    ...
    public Param getReceiver() {
        if (receiver == null) {
            this.receiver = new Param(this, getSig().getReceiverType(), -1);
        }
        return receiver;
    }
