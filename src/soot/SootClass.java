...
public class SootClass extends AbstractHost implements Numberable, javarifier.Signatured
{
    ...
    private ClassSig classSig;
     public ClassSig getSig() {
       if(classSig == null) {
         TypeInitializer.loadSignature(this);
         if(classSig == null) {
           throw new RuntimeException("SootClass: TypeInitializer failed to load this signature");
         }
       }
         return classSig;
     }
    public void setSig(ClassSig cs) {
        classSig = cs;
    }

    private SourceLocator.EntryKind entryKind = null;
    public SourceLocator.EntryKind entryKind() {
      if(entryKind == null) {
        entryKind = SourceLocator.v().entryKindForClass(name);
      }
        return entryKind;
    }
    public void setEntryKind(SourceLocator.EntryKind entryKind) {
        this.entryKind = entryKind;
    }
    
    private Boolean isStatic;
    private boolean checkIsStatic() {
        if (hasOuterReference) {
            for (SootField f: (Collection<SootField>) getFields())
    ...
    /**
	 * Returns true if this class is a static inner class or a toplevel class.
         * May incorrectly return "false" if the class is not resolved to
         * signatures.
	 */
    public boolean isStatic() {
    	if (isStatic == null) {
            if (resolvingLevel >= SIGNATURES)
                isStatic = checkIsStatic();
            else
                return !hasOuterReference;
        }
        return isStatic;
    }
    ...
    */
     ...
     * 
     * @param subsignature the signature of the method, including the return type
     * @return the m
     */
    public SootMethod getMethodIgnoringReturnType(NumberedString subsignature) {
      checkLevel(SIGNATURES);
      String subsignatureMinusReturn = stripReturnType(subsignature);
      SootMethod ret = null;
      
      Iterator/*<NumberedString>*/ itr = subSigToMethods.keyIterator();
      while(ret == null && itr.hasNext()) {
        NumberedString ns = (NumberedString) itr.next();
        String nss = stripReturnType(ns);
        if(nss.equals(subsignatureMinusReturn)) {
          ret = (SootMethod) subSigToMethods.get(ns);
        }
      }
      return ret;
    }
    ...
    /**
     * Utility method to remove the return type from a subsignature.
     * @param subsignature the signature of the method, including the return type,
     * such as "java.lang.Object get()"
     * @return a string with the return type removed, such as "get()"
     */
    private String stripReturnType(NumberedString subsignature) {
    ...
    */
    ...
    {
        List<soot.Type> allTypes = Arrays.asList(
                soot.coffi.Util.v().jimpleTypesOfFieldOrMethodDescriptor(descriptor));
    ...
    */
    ...
    private void checkOuterClass() {
        if (outerStuffChecked < 1) {
            outerClass = null;
            hasOuterReference = false;
            outerMethodName1 = null;
            outerMethodDesc1 = null;
                ...
                public void visitInnerClass(String className, String outerName, String innerName, int access) {
                    ...
                    if (className.equals(getName())) {
                        if (outerName != null) {
                            ...
                            outerClass = RefType.v(outerName).getSootClass();
                        }
                        hasOuterReference = (access & Opcodes.ACC_STATIC) == 0;
                    }
                }

                @Override
                public void visitOuterClass(String outerName, String methodName, String desc) {
                    outerName = outerName.replace('/','.');
                    outerClass = RefType.v(outerName).getSootClass();
                    outerMethodName1 = methodName;
                    outerMethodDesc1 = desc;
                }
            };
            ASMClassReaders.v().readerFor(getName()).accept(cv, true);
            outerStuffChecked = 1;
        }
    }
    // End javarifier changes
    ...
    private void checkOuterMethod() {
        if (outerStuffChecked < 2) {
            checkOuterClass();
            if (outerMethodName1 == null)
                outerMethod = null;
            else {
              // Classes not resolved to at least signatures don't need to 
              // know about their containing method.  (E.g. Anonymous
              // private classes in stub classes.)
              if(resolvingLevel < SootClass.SIGNATURES) {
                outerMethod = null;
              } else {
                outerMethod = outerClass.getMethodByDescriptor(
                        outerMethodName1, outerMethodDesc1);
              }
            }
            outerStuffChecked = 2;
        }
