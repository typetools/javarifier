...
public class StubConstraints extends SceneVisitor {
    ...
    public void visitJrTyped(JrTyped typed) {
        if (typed.getJrType() != null) {
            StubConstraintsHelper.generate(typed, cm);
        }
    }
    ...
    public static class StubConstraintsHelper extends EmptyTypeVisitor {
        ...
        public static void generate(JrTyped typed, ConstraintManager cm) {
          ...
          if(type instanceof MutType) {
            ...
            typed.getJrType().accept(new StubConstraintsHelper(typed, cm));
        }

        private JrTyped val;
        private ConstraintManager cm;

        public StubConstraintsHelper(JrTyped val, ConstraintManager cm) {
            this.val = val;
            this.cm = cm;
        }
        ...
        private void generateConstraints(MutType type) {
            ...
            case MUTABLE :
            case THIS_MUTABLE :
                cm.mutable2(val, type, Context.READONLY);
                cm.mutable2(val, type, Context.MUTABLE);
                break;
            case POLYREAD :
                cm.mutable2(val, type, Context.MUTABLE);
                break;
