 ...
 */
public class ShayOutput<A extends SimpleAnnotation> {
  ...
  public static void write(AScene<SimpleAnnotation> ascene, Writer out) 
  throws IOException, DefException {

    ShayOutput<SimpleAnnotation> so = new ShayOutput<SimpleAnnotation>(out);
...
//  }
  ...
  private void printScene(AScene<A> ascene) throws IOException {
    allClasses = new ArrayList<String>();
    polyreadStrings = new ArrayList<String>();
    className = "INVALID_CLASS";
    for(Map.Entry<String, AClass<A>> e : ascene.classes.entrySet()) {
  ...
  private void printClass(AClass<A> c) throws IOException {
    methodName = "INVALID_METHOD";
    for(Map.Entry<String, AMethod<A>> e : c.methods.entrySet()) {
  ...
  private void printMethod(AMethod<A> m) throws IOException {
    printReceiver(m.receiver);
    
    for(Map.Entry<Integer, ATypeElement<A>> e : m.parameters.entrySet()) {
      printParam(e.getKey(), e.getValue());
    }
  }
  
  private void printReceiver(AElement<A> r) throws IOException {
  ...
  private void printParam(int index, ATypeElement<A> param) throws IOException {
...
//      out.write("\n");
  ...
  private boolean isReadOnly(AElement<A> element, boolean checkArray) {
    for(TLAnnotation<A> tla : element.tlAnnotationsHere) {
      String annotationName = tla.ann.def().name;
    ...
    if(checkArray && element instanceof ATypeElement) {
      ATypeElement<A> ae = (ATypeElement<A>) element;
      List<Integer> loc = new ArrayList<Integer>();
      loc.add(0);
      InnerTypeLocation firstLoc = new InnerTypeLocation(loc);
      AElement<A> innerElement = ae.innerTypes.vivify(firstLoc);
  ...
  private boolean isPolyRead(AElement<A> element) {
    for(TLAnnotation<A> tla : element.tlAnnotationsHere) {
      String annotationName = tla.ann.def().name;
